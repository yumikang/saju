...
export async function searchHanjaFromDB(
  ...
  if (sort === 'popularity') {
    // Null-safe popularity 정렬: NULL과 0을 뒤로 보내고, 값이 있는 것은 내림차순
    const query = Prisma.sql`
      SELECT 
        ...
        "evidenceJSON"
      FROM "HanjaDict"
      WHERE character = ANY(${characters})
      ${cursor ? Prisma.sql`AND id > ${cursor}` : Prisma.empty}
      ORDER BY 
        CASE 
          WHEN "nameFrequency" IS NULL OR "nameFrequency" = 0 THEN 1
          ELSE 0
        END,
        "nameFrequency" DESC NULLS LAST,
        "usageFrequency" DESC NULLS LAST,
        id ASC
      LIMIT ${actualLimit}
    `;
    ...
    // Null-safe strokes 정렬: NULL과 0을 뒤로 보내고, 값이 있는 것은 오름차순
    const query = Prisma.sql`
      SELECT 
        id, 
        character, 
      ...
      ORDER BY 
        CASE 
          WHEN strokes IS NULL OR strokes = 0 THEN 999999
          ELSE strokes
        END ASC,
        id ASC
      LIMIT ${actualLimit}
    ...
    // Null-safe element 정렬: NULL을 뒤로 보내고, 값이 있는 것은 알파벳순
    const query = Prisma.sql`
      SELECT 
        id, 
        character, 
      ...
      ORDER BY 
        CASE 
          WHEN element IS NULL THEN 'ZZZZZ'
          ELSE element::text
        END ASC,
        id ASC
      LIMIT ${actualLimit}
