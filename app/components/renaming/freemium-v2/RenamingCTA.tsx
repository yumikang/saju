/**
 * RenamingCTA Component - Strategic Freemium V2 for Renaming Service
 *
 * Conversion-optimized call-to-action between free (11-12위) and locked (1-10위) sections.
 * Psychological optimization strategies adapted for renaming:
 * - Current name vs new name improvement emphasis
 * - Score difference emphasis (1위 vs 11위)
 * - Value proposition (10 premium names)
 * - Trust elements (환불 보장, 평생 이용)
 * - Life transformation focus (new beginning)
 *
 * Features:
 * - Animated gradient background
 * - Pulsing lock icon
 * - Score comparison display with improvement from current name
 * - Price with discount badge (₩120,000)
...
export interface RenamingCTAProps {
  /** Renaming psychological metrics from classification */
  metrics: RenamingPsychologicalMetrics;

  /** Payment button click handler */
  onPayment: () => void;

  /** Optional custom price (default: 120000) */
  price?: number;
}

/**
 * RenamingCTA Component
 */
    ...
    >
      <Card className="relative overflow-hidden border-4 border-yellow-400 shadow-2xl">
        ...
        <div className="relative p-6 sm:p-8">
          ...
          {/* Main message - score comparison with improvement */}
          <div className="text-center mb-6">
            ...
            {/* Current name improvement emphasis */}
            {metrics.improvementFromCurrent && metrics.improvementFromCurrent >= 10 && (
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.3 }}
                className="mb-4 inline-block bg-gradient-to-r from-green-100 to-emerald-100 px-5 py-3 rounded-xl border-2 border-green-300"
              >
                <p className="text-lg font-bold text-green-700 flex items-center gap-2 justify-center">
                  <TrendingUp className="w-5 h-5" />
                  현재 이름보다 <span className="text-2xl text-green-600">{metrics.improvementFromCurrent}점</span> 개선!
                </p>
              </motion.div>
            )}

            <p className="text-base sm:text-lg text-gray-700 mb-2">
              지금 보신 <strong className="text-emerald-600">11-12위 무료 개명</strong>도 좋지만,
            </p>
            <p className="text-base sm:text-lg text-gray-700 mb-2">
              <strong className="text-orange-600">
                1-10위 프리미엄 개명은 평균 {metrics.scoreDifference}점 더 높은
              </strong>{' '}
              완벽한 조화입니다
            </p>

            <p className="text-sm text-gray-600 mt-3">
              💡 평생 사용할 소중한 새 이름, 최고 점수 10개 중에서 선택하세요
            </p>
          </div>
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-xl p-5 sm:p-6 mb-6 border-2 border-yellow-200">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-gray-900">
                    ...
                    프리미엄 10개 개명 (1-10위) 잠금 해제
                  </p>
                  <p className="text-sm text-gray-600">
                    최고 점수 {metrics.topScore}점부터 상위 10개 개명 공개
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-gray-900">
                    전체 {metrics.totalCount}개 개명 + 상세 분석
                  </p>
                  <p className="text-sm text-gray-600">
          ...
          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            ...
            >
              ...
              지금 바로 1-10위 프리미엄 개명 보기 · ₩{price.toLocaleString()}
            </Button>
          </motion.div>
          ...
          <div className="mt-4 text-center">
            <p className="text-xs text-gray-500">
              💡 인생의 새 출발을 위한 투자, 개명 하나당 약 ₩{pricePerName.toLocaleString()}
            </p>
          </div>
        </div>
      </Card>
